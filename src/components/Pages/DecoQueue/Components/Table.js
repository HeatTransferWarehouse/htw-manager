  ...
  const filteredItems = currentViewItems.filter((item) => {
    ...
    if (searchQuery.includes(':')) {
      ...
      searchMatch = Object.keys(searchMap).every((key) => {
        if (key === 'sku') {
          return searchMap[key].some((value) => item.sku.toLowerCase().includes(value));
        } else if (key === 'id') {
          return searchMap[key].some((value) => item.order_number.toString().includes(value));
        } else if (key === 'name') {
          return searchMap[key].some((value) => item.description.toLowerCase().includes(value));
        } else if (key === 'date') {
          return searchMap[key].some((value) => item.created_at.toLowerCase().includes(value));
        } else if (key === 'qty') {
          return searchMap[key].some((value) => item.qty.toString().toLowerCase().includes(value));
        }
        return false;
      });
    } else {
