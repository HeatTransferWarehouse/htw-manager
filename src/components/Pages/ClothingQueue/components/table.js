    ...
    if (searchQuery.includes(':')) {
      ...
      searchMatch = Object.keys(searchMap).every((key) => {
        if (key === 'sku') {
          ...
          return searchMap[key].some((value) => (item.order_id ?? '').toString().includes(value));
        } else if (key === 'name') {
          return searchMap[key].some((value) =>
            (item.description ?? '').toLowerCase().includes(value)
          );
        } else if (key === 'date') {
          return searchMap[key].some((value) => (item.date ?? '').toLowerCase().includes(value));
        } else if (key === 'qty') {
          return searchMap[key].some((value) =>
      ...
      <TableContainer tableFor={'clothing'}>
        {!props.isMobile && (
          <TableHeader>
            ...
            <TableHeadCell minWidth={'7rem'}>{renderSortButton('color', 'Color')}</TableHeadCell>
            <TableHeadCell minWidth={'7rem'}>{renderSortButton('size', 'Size')}</TableHeadCell>
            <TableHeadCell>{renderSortButton('qty', 'Qty')}</TableHeadCell>
            <TableHeadCell minWidth={'8rem'}>{renderSortButton('date', 'Order For')}</TableHeadCell>
            <TableHeadCell />
