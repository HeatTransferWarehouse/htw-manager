  ...
  const filteredProducts = useMemo(() => {
    ...
    return imageProducts.data.filter((product) => {
      ...
      const matchesIssueFilter =
        issueFilter === 'all' ||
        (issueFilter === 'missing' && product.issue.toLowerCase().includes('missing')) ||
        (issueFilter === 'duplicate' && product.issue.toLowerCase().includes('duplicate'));
          ...
          <>
            {isMobile ? (
              ...
              <TableContainer tableFor={'productsListImages'}>
                ...
                <TableBody>
                  {paginatedProducts.map((product) => (
                    <TableRow className={'last:border-b'} key={product.product_id}>
                      <TableCell>
                        <a
                          className="hover:text-secondary underline"
                          target="_blank"
                      ...
                      <TableCell className={'pr-4 pl-2 grid grid-cols-5 gap-1'}>
                        {product.images.map((image, index) => (
                          <Image key={`${image.id}-${index}`} url={image.image_url} />
                        ))}
                      </TableCell>
                      <TableCell className={'!p-2'}>
                        <span
                          className={twMerge(
                            product.issue === 'Missing Alt' ? 'bg-red-200' : 'bg-blue-200',
