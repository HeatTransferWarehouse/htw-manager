...
function* splitOrder(action) {
  try {
    ...
    yield put({ type: 'GET_HTW_ORDERS', payload: { page, filter: view, limit: rowsPerPage } }); // Refresh orders
  } catch (err) {
    yield put({
    ...
    yield delay(5000);
    yield put({ type: 'CLEAR_HTW_ORDERS_ERROR' });
...
function* combineOrders(action) {
  try {
    ...
    yield put({ type: 'GET_HTW_ORDERS', payload: { page, limit: rowsPerPage, filter: view } }); // Refresh orders
  } catch (err) {
    yield put({
      type: 'SET_HTW_ORDERS_ERROR',
      payload: {
...
function* getOrders(action) {
  try {
    ...
    if (loadingDelay) {
      ...
      yield put({ type: 'SET_HTW_ORDERS_LOADING', payload: true });
    ...
    yield put({ type: 'SET_HTW_ORDERS_LOADING', payload: false });
  }
}
...
function* syncOrders() {
  try {
    yield put({ type: 'SET_HTW_SYNCING', payload: true });
    yield axios.post(`/api/big-commerce/orders/sync`, {
      storeKey,
    });
    yield put({ type: 'GET_HTW_ORDERS' }); // Refresh orders after sync
  } catch (err) {
    yield put({
      type: 'SET_HTW_ORDERS_ERROR',
      payload: {
    ...
    yield put({ type: 'SET_HTW_SYNCING', payload: false });
  }
}
...
function* deleteOrders(action) {
  try {
    ...
    yield put({ type: 'GET_HTW_ORDERS' }); // Refresh orders
  } catch (err) {
    yield put({
      type: 'SET_HTW_ORDERS_ERROR',
      payload: {
...
function* htwOrdersSaga() {
  yield takeLatest('GET_HTW_ORDERS', getOrders);
  yield takeLatest('MARK_HTW_ORDERS_PRINTED', markOrdersPrinted);
  yield takeLatest('GENERATE_HTW_PDF', generatePDF);
  yield takeLatest('SYNC_HTW_ORDERS', syncOrders);
  yield takeLatest('GET_HTW_PRINTERS', getLocalPrinters);
  yield takeLatest('DELETE_HTW_ORDERS', deleteOrders);
  yield takeLatest('GET_HTW_TAGS', getTags);
  yield takeLatest('SPLIT_HTW_ORDER', splitOrder);
  yield takeLatest('COMBINE_HTW_ORDERS', combineOrders);
  yield takeLatest('ADD_HTW_ORDER', addOrder);
}

export default htwOrdersSaga;
